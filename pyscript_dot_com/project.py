...
class AsyncDatastore(BaseDataStore):
    ...
    async def get(self, key: str):
        """Get a value from datastore."""
        result = async_request(f"{self.api_base}/datastore/{key}")
    ...
    async def delete(self, key: str):
        """Delete a value from datastore."""
        async_request(f"{self.api_base}/datastore/{key}", method="DELETE")
        # We can't be sure that the key is in self._data so let's
        # just suppress the error
        with contextlib.suppress(KeyError):
            del self._data[key]
    ...
    async def items(self):
        ...
        # In this case we should always hit the DB and
        # return the latest data, plus updating `self._data`
        result = async_request(f"{self.api_base}/datastore")
        if self._is_api_error(result):
            return []
        self._data = result
        return list(self._data.items())
    ...
    async def setdefault(self, key: str, default=None):
        """Implement setdefault method."""
    ...
    async def pop(self, key, default=None):
        """Pop the specified item from the data store."""
        result = async_request(
            f"{self.api_base}/datastore/{key}?pop=true", method="DELETE"
        )
        if self._is_api_error(result):
            return default
    ...
    async def update(self, *args, **kwargs):
        ...
        result = async_request(
            f"{self.api_base}/datastore", method="PUT", body=new_items
        )
    ...
    async def paginate_items(self, count: int = 10):
        """Paginate items in datastore."""
        result = async_request(f"{self.api_base}/datastore", params={"count": count})
        if self._is_api_error(result):
            return []
        self._data = result
        return list(self._data.items())
...
datastore = Datastore()
async_datastore = AsyncDatastore()
